...
function colorizeWord(word, tpl = {}) {
    const {
        color = 'white',
        style = 'normal',
        back = 'none'
    ...
    res += `;${colors[color]}m${word}\x1b[0m`;

    return res;
}

function colorize(str, config = defConfig) {
    ...
    const result = tokens.map(token => {
        ...
        return colorizeWord(word, config[index]);
    });
...
function log(message, config) {
    console.log(colorize(message, config));
}

class ColoredError extends Error {
    constructor(message, config) {
        super(colorize(message, config));
        this.name = colorize('#3{Error}');
    }
}

module.exports = { log, ColoredError };